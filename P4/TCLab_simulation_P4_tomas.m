...
%% Q4 following a referência using the feedforward scheme

%Steady State computation step - using steady state model
function [Duss,Dxss,Dr]=SS_comp(Dr,A,B,C) 
...
[Duss,Dxss,Dr]=SS_comp(15,A,B,C); 

%% experiment 1 - unperturbed
...
fprintf('Q4 — H=%d, R=%.3f (constrained) ...\n', H_chosen, R_Q3)

t  = nan(1,N);  y  = nan(1,N);
Dy = nan(1,N);  Du = nan(1,N);
dx = nan(n,N+1);  dy = nan(1,N); du = nan(1,N);
u  = nan(1,N);
x  = nan(n,N+1);
x(:,1) = x0;
...
for k = 1:N
    ...
    % Constrained: lb and ub passed, no clipping needed
    du(k)    = mpc_solve(dx, H_chosen, R_Q3, A, B, C, lb, ub);
    Du_k=du(k)+Duss;
    u(k)    = u_ss + Du_k;
    Du(k)   = Du_k;
...
figure('Units','normalized','Position',[0.2 0.05 0.3 0.4])
subplot(2,1,1), hold on, grid on
title(sprintf('Q4 — Absolute  (H=%d, R=%.3f, constrained)', H_chosen, R_Q3))
plot(t, y, '.', 'MarkerSize', 5)
yl = yline(y_ss,'k--');
xlabel('Time [s]'), ylabel('y [°C]')
legend(yl,'$\bar{y}$','Interpreter','latex','Location','best')
...
figure('Units','normalized','Position',[0.5 0.05 0.3 0.4])
subplot(2,1,1), hold on, grid on
title(sprintf('Q4 — Incremental  (H=%d, R=%.3f, constrained)', H_chosen, R_Q3))
plot(t, Dy, '.', 'MarkerSize', 5)
yline(0,'k--')
xlabel('Time [s]'), ylabel('\Delta y [°C]')
subplot(2,1,2), hold on, grid on
